...
def prepare_data_for_train() -> Tuple[pd.DataFrame]:
    ...
    #users_data = interactions.drop(columns=['movie_id', 'year', 'month', 'day', 'watch_duration_minutes', 'watched_ratio'])
    users_data = interactions
    # pred candidates
    preds = users_data[["user_id"]].drop_duplicates().reset_index(drop=True)
    ...
    preds["movie_id"] = preds["user_id"].map(mapper)

    # define target & prepare ranker sample
    preds = preds.explode("movie_id")
    logging.info(f"Shape of the preds: {preds.shape}")

    preds["rank"] = preds.groupby("user_id").cumcount() + 1
    logging.info(
        f"Number of unique candidates generated by the LFM: {preds.movie_id.nunique()}"
    )

    # train_data = get_ranker_sample(preds=preds, users_sample=users_sample)
    train_data = get_ranker_sample(preds=preds, users_sample=users_data)
...
def get_ranker_sample(preds: pd.DataFrame, users_sample: pd.DataFrame):
    ...
    # prepare train & test
    positive_preds = pd.merge(preds, local_test, how="inner", on=["user_id", "movie_id"])
    positive_preds["target"] = 1
    logging.info(f"Shape of the positive target preds: {positive_preds.shape}")

    negative_preds = pd.merge(preds, local_test, how="left", on=["user_id", "movie_id"])
    ...
    cbm_train_set = pd.merge(
        cbm_train_set,
        items_data[["movie_id"] + settings.ITEM_FEATURES],
        how="left",
        on=["movie_id"],
    )
    cbm_test_set = pd.merge(
...
def get_items_features(item_ids: List[int], item_cols: List[str]) -> Dict[int, Any]:
    ...
    item_features = read_parquet_from_gdrive(
        "https://drive.google.com/file/d/12R_q0TncwtX9ugE2nUbejIsASU3lS67s/view?usp=share_link"
    )
    # item_features = item_features.set_index("movie_id")
    item_features = item_features.set_index("movie_id")
    item_features = item_features.to_dict("index")
